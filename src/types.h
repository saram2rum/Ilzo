...
enum Piece : int { PAWN, KNIGHT, BISHOP, ROOK, QUEEN, KING, PIECE_NB };

constexpr inline void setBit(Bitboard& bb, int sq) { bb |= (1ULL << sq); }
constexpr inline void clearBit(Bitboard& bb, int sq) { bb &= ~(1ULL << sq); }
constexpr inline bool checkBit(Bitboard bb, int sq) { return (bb & (1ULL << sq)) != 0; }
